...
export interface AppState {
  ...
  /** Iteration the next generation descends from, if it was forked/re-rolled. */
  pendingIterationParentId: string | null;
  /** Whether that next generation opens its own branch. */
  pendingIterationNewBranch: boolean;
  ...
  /** Record which iteration the next generation descends from (creates no node). */
  setPendingIterationParent: (parentId: string | null, options?: { newBranch?: boolean }) => void;
  /** Forget any recorded lineage for the next generation. */
  clearPendingIterationParent: () => void;
  ...
  analyzeUntaggedAssets: () => void;
  /** Route new assets by the user's `taggingMode` (analyse now, queue, or skip). */
  enqueueForTagging: (assetIds: string[]) => void;
  /** Re-evaluate every smart collection against the current library + metadata. */
  refreshAllSmartCollections: () => void;
