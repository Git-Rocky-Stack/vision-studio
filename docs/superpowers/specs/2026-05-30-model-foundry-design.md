...
1. **Fast transfers.** Enable `hf_transfer` (`HF_HUB_ENABLE_HF_TRANSFER=1`) for parallel chunked pulls on large weights, with a clean fallback when absent/unsupported (add to `requirements.txt`). `hf_transfer` trades progress granularity for speed → default **on** for large Verified pulls, with a "precise progress" fallback for byte-exact bars. *(Mechanism proven in Spike A.)*
2. **True progress streaming.** Pre-compute total bytes from repo metadata (`HfApi.model_info(files_metadata=True)` / `list_repo_files`), download **per-file**, accumulate completed + in-flight bytes into the `ModelRecord` (`progress`, `speed`, `eta`). File-granularity minimum; byte-granularity on the precise path. Replaces the fake 0→100.
3. **`DownloadManager`.** Bounded-concurrency queue (D5: configurable, default 2, clamp 1–6), each job an `asyncio.Task` keyed by model id — wiring the dormant `download_tasks` dict. Lifecycle: **pause** (cancel task, keep partials), **resume** (re-invoke with `resume_download`), **cancel** (stop + clean partials), **reorder / priority**.
...
5. **Atomic, verified completion.** Download to `.incomplete` → `verifying` (sha256 / HF etag) → atomic move → register `ready`. An interrupted/corrupt download never presents as `ready`. Brings HF single-file and diffusers-bundle paths under the temp+move discipline the CivitAI path already uses.
...
| `hf_transfer` ↔ progress granularity tension | Documented precise-mode fallback; **mechanism chosen in Spike A** |
