...
| P1 - CSP blocks generated/imported media | RESOLVED (fully, 2026-06-08) | The original fix patched only the **session-header** CSP (`electron/services/contentSecurityPolicy.ts`) and was wrongly marked "confirmed". The redundant `<meta http-equiv="Content-Security-Policy">` in `index.html` still carried the stale `img-src 'self' data: blob:`; the browser enforces the **intersection**, so generated media stayed blocked in the running app. Surfaced by the new completion E2E (`naturalWidth:0`, explicit CSP violation in console). Meta synced to the header and locked by a meta-matches-header regression test in `contentSecurityPolicy.test.ts`. |
...
| P0 - Dependency security audit is red | RESOLVED (2026-06-08) | Shipped tree is clean: `react-router-dom` 7.13.0 -> 7.17.0 cleared the last shipped advisories; `npm audit --omit=dev` reports **0**. The 10 residual vulns are all dev-only (electron-builder packaging tree + dev `brace-expansion`), documented in `docs/dependency-security.md` and gated by a new `audit-prod` CI job. The breaking electron-builder 25->26 bump is deferred to a dedicated effort. |
| P0 - Backend/E2E gates not release-trustworthy | RESOLVED (2026-06-08) | Backend pytest gate + benchmark isolation fixed; pytest tiers added (`unit`/`integration`, conftest auto-tag). The mocked-backend Electron E2E completion test now exists (`tests/e2e/generate-completion.spec.ts`) and proves submit -> progress -> completed -> asset record -> **rendered preview** - it caught the CSP meta-tag bug above. |
| P2 - Signing / bundle size / accessibility | RESOLVED bundle+a11y; DOCUMENTED signing (2026-06-08) | **Accessibility:** smoke suite passes with **zero** exceptions - `nested-interactive` (ControlNetPanel) and `color-contrast` (text-muted/60) fixed, plus three pre-existing Generate/Timeline violations (`button-name`, `aria-required-children`, `scrollable-region-focusable`). **Bundle:** entry chunk 825 -> 621 kB (-25%) via route/panel `React.lazy` splitting; the >800 kB warning is cleared by real splitting, not a raised limit. **Signing:** `release.yml` preflight wiring verified and `docs/code-signing.md` corrected (accurate secrets + explicit dev-unsigned vs prod-signed); setting the GitHub secret **values** for one mode remains the repo owner's action. |
