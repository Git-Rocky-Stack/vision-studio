...
def run_migrations(db_path: str) -> None:
    ...
    if not MIGRATIONS_DIR.exists():
        print(f"⚠️  Migrations directory not found: {MIGRATIONS_DIR} — skipping migrations")
        return
        ...
        for version, filename in pending:
            try:
                ...
                if not hasattr(migration_module, 'migrate_up'):
                    print(f"⚠️  Migration {filename} missing migrate_up — skipping")
                    continue
                ...
                print(f"⚠️  Migration {filename} failed: {e} — skipping")
                continue
    finally:
        conn.close()
