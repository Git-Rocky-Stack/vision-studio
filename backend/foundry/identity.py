...
def quick_identity(path: str) -> str:
    """'<size>:<first 16 hex of sha256(head||tail)>' — stable, collision-cheap. Raises OSError if the file is unreadable."""
...
def full_sha256(path: str) -> str:
    """Full streaming sha256 (lazy/background use). Raises OSError if unreadable."""
    digest = hashlib.sha256()
    with open(path, "rb") as handle:
